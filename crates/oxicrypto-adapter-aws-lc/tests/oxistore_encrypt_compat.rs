...
//! ## What this verifies
//!
//! - `AwsLcOxistoreAead` — a thin newtype wrapping `AwsLcAead` (oxicrypto-adapter-aws-lc)
//!   that implements the `oxistore_encrypt::aead::Aead` trait.
...
//! newtype bridges the gap by delegating to the `oxicrypto_core::Aead` methods
//! exposed on `AwsLcAead` while adapting inputs and outputs to the store API.
...
mod tests {
    ...
    use oxistore_encrypt::aead::Aead as StoreAead;
    use oxistore_encrypt::{EncryptError, EncryptedKv, StaticKey};
    ...
    // ── AwsLcOxistoreAead — bridge newtype ────────────────────────────────────

    /// Bridge adapter that wraps [`AwsLcAead`] and implements the
    /// `oxistore_encrypt::aead::Aead` trait.
    ///
    /// This allows `EncryptedKv::with_aead(inner, key, AwsLcOxistoreAead::…())` to
    /// use the aws-lc-rs AEAD implementation as the cell-level encryption backend
    /// instead of the default XChaCha20-Poly1305 RustCrypto path.
    #[derive(Debug, Clone, Copy)]
    struct AwsLcOxistoreAead {
        inner: AwsLcAead,
    }

    impl AwsLcOxistoreAead {
        /// AES-256-GCM-SIV via aws-lc-rs (12-byte nonce, 32-byte key, misuse-resistant).
        fn aes256_gcm_siv() -> Self {
            Self {
